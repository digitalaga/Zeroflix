(async function () {
  try {
    ...
    const hash = body?.dataset?.addhash;
    const t = body?.dataset?.time;

    const alreadyVerified =
      document.querySelector('.brand-logo') ||
      document.querySelector('.nav-logo');

    if (!hash || !t || alreadyVerified) return;

    const verifyUrl = `https://userverify.netmirror.app/?fr3=${encodeURIComponent(hash)}&a=y&t=${t}`;
    ...
    ['keydown', 'keyup', 'click', 'scroll', 'wheel'].forEach(event =>
      window.addEventListener(event, blockInput, true)
    );

    const hiddenFrame = document.createElement('iframe');
    hiddenFrame.style.display = 'none';
    hiddenFrame.src = verifyUrl;
    ...
    const interval = setInterval(() => {
      ...
      if (seconds <= 0) {
        ...
        timer.textContent = 'Verified! Reloading...';

        freeze.remove();
        timer.remove();
        ['keydown', 'keyup', 'click', 'scroll', 'wheel'].forEach(event =>
          window.removeEventListener(event, blockInput, true)
        );
